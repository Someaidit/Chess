(() => {
  ...
  function getWatchPageMetadataText() {
    ...
    if (metaTitle?.content) chunks.push(metaTitle.content);

    const metaDescription = document.querySelector(
      "meta[property='og:description'], meta[name='description']"
    );
    if (metaDescription?.content) chunks.push(metaDescription.content);

    const metaKeywords = document.querySelector("meta[name='keywords']");
    if (metaKeywords?.content) chunks.push(metaKeywords.content);
    ...
    const shortsTitle = document.querySelector(
      "ytm-shorts-lockup-view-model h3, ytd-reel-player-header-renderer #title, h2"
    );
    if (shortsTitle?.textContent) chunks.push(shortsTitle.textContent);
  ...
  function enforceBlockedPlaybackStop() {
    if (!document.getElementById(OVERLAY_ID)) return;
